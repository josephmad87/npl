...
export type DismissalKind = 'dnb' | 'not_out' | 'retired_hurt' | 'out' | 'empty'
...
export function dismissalKindFromValue(dismissal: string | null | undefined): DismissalKind {
  const t = normalizeDismissal(dismissal)
  if (!t) return 'empty'
  if (t === DID_NOT_BAT) return 'dnb'
  if (t === NOT_OUT) return 'not_out'
  if (t === RETIRED_HURT) return 'retired_hurt'
  return 'out'
}

export function dismissalValueFromKind(
  kind: DismissalKind,
  detail: string,
): string {
  switch (kind) {
