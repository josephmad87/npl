...
export default function MerchandiseOrderTrackingPage() {
  ...
  const contentQ = useSitePageContent('order-tracking')
  const updatesContent = managedSection(contentQ.data, 'updates', 'Updates')
  return <main className="container merchandise-tracking-page">
    <SeoHead title={contentQ.data?.title || 'Track Order'} description={contentQ.data?.subtitle || 'Private NPL merchandise order tracking.'} canonicalPath={`/merchandise/orders/${orderNumber}`} noIndex />
    <Link to="/merchandise">← Back to merchandise</Link>
    <h1>{contentQ.data?.title || 'Track Order'}</h1>
    {trackingQ.isLoading ? <p>Loading order…</p> : trackingQ.isError || !trackingQ.data ? <p className="form-error">This tracking link is invalid or has expired.</p> : <>
      ...
      <section className="merchandise-tracking-page__timeline"><h2>{updatesContent.heading}</h2><ol>{trackingQ.data.timeline.map((event, index) => <li key={`${event.created_at}-${index}`}><strong>{event.status.replaceAll('_', ' ')}</strong>{event.public_message ? <p>{event.public_message}</p> : null}<time>{new Date(event.created_at).toLocaleString()}</time></li>)}</ol></section>
    </>}
  </main>
}
